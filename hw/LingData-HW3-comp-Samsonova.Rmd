...
ANSWER: there are 5 values that exceed 5 and there are 20 values in the sample so 5/20 = 0.25
the proportion is 25%

b. Indicate the interval where the median of this sample can lie. Explain your answer.
ANSWER: there are 20 values in the sample, the sample is ranked ascending, the number of elemrnts in the sample is even, so the median lies between the 10th and the 11th elements. so it belongs to the interval [-5,0]

c. How the histogram will change if we add an element $7$ to the sample? Explain your answer.
if we add an element 7 to the sample the bar that is in an interval [5,10] will increase 
...
В параметрах нормального распределения в качестве третьего аргумента передается стандартное отклонение, сдандарстное отклонение является квадратным корнем дисперсии, следовательно диспесия больше у той выборки, у которой больше стандартное отклонение, здесь это первая выборка. Также можно просто посмотреть на степень отклонеения от мат.ожидания, на гистограме это "разброс значений", так первая выборка выходит за значения 400 и 600 в то время как вторая выборка лежит внутри этого отрезка, следовательно дисперсия в первой выборке выше.
\newpage
...
Which of the samples has a larger variance? Explain your answer.
у первой выборки (по аналигии с объяснением из предыдущего задания по стандартному отклонению) Также видим, что "разброс" принимаемых значений в первой выборке лежит в промежутке [-15,15], то есть случайная величина принимает значения, описываемые множеством мощностью около 30 элементов, в то время как во второй выборке случайная величина принимает значение от 8 до 12 (мощность множетсва значений элементарных исходов 5)
...
площадь под графиков правее значения 35 меньше 50 процентов всей площали => и количество студентов, совершивших больше 35 ошибок меньше 50 %
...
```
Explain your answer below:
Пусть большинство студентов - это >=51% 
площадь графика меньше 10 ошибок меньше 51 процента, следовательно большая часть студентов совершила большее количество ошибок
...
```
Explain your answer below:
Медиана - это такое значение случайной величины, что если ранжировать все элементы выборки по возрастанию, то справа и слава от медианы будет располагаться половина всех элементов выборки, следовательно это такая прямая, которая разделит площадь графика пополам, по графику можно сказать, что это примерно 20 
...
words.relfreq = words.freq/length(words)
words.relfreq
```
...
data_1920 <- data[data$Decade == "1920s",]
Verbs <- data_1920[data_1920$UPoS == "VERB",]
Sample_length <- nrow(data_1920)
Sample_length
Number_of_verbs <- nrow(Verbs)
Number_of_verbs
...
```{r}
binom.test(Number_of_verbs, Sample_length, p=0.17)
```
...
```
p-value больше 0,05, следовательно мы отвергаем 0 гипотезу 
```
...
```{r}
data_1820 <- data[data$Decade == "1820s",]
verbs <- data_1820[data_1820$UPoS == "VERB",]
Sample_length <- nrow(data_1820)
Number_of_verbs <- nrow(verbs)
binom.test(Number_of_verbs, Sample_length, p=0.17)
```
...
```
H1: the population mean of vowel duration in speaker `shg05` is not equals 73
```

### 8.2
Perform a one-sample t-test. 
```{r}
data_islandic <- read_csv("https://raw.githubusercontent.com/LingData2019/LingData2020/master/data/icelandic.csv")
sample_shg05 <- data_islandic[data_islandic$speaker == "shg05",]
sample_mean <- mean(data_islandic$vowel.dur)
...
Interpret results. 
```
p-value значительно меньше 0,05, следовательно мы подтвержаем 1 гипотезу
```
